...
#ranew
class BuggyChangelistDetectorMaster(service.MultiService):
    """ To assist the process of finding the buggy changelist by executing selected tasks and check the result manually"""
    debug = 0
    manhole = None
    debugPassword = None
    projectName = "(unspecified)"
    ...
    def __init__(self, basedir, configFileName="detector.cfg"):
        service.MultiService.__init__(self)
        self.setName("detectormaster")
        self.basedir = basedir
        self.configFileName = configFileName
    ...
    def startService(self):
        ...
        if not self.readConfig:
            ...
            #racom: self.loadTheConfigFile()
            #ranew: 
            # * the method call is load the desired steps
            # * I think only allow the master to control the debug step, at 
            #   least seemingly.  
            # * load another config file in case of polluting static one.
            self.dynamicReloadConfigFile() 
    ...
    # 
    def dynamicReloadConfigFile(self, configFile=None):
        if not configFile:
            configFile = os.path.join(self.basedir, self.configFileName)

        log.msg("Creating BuggyChangelistDectorMaster -- buildbot.version: %s" % buildbot.version)
        log.msg("loading configuration from %s" % configFile)
        configFile = os.path.expanduser(configFile)
