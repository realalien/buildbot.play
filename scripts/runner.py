...
#ranew20100203
class DetectorMasterOptions(MakerBase): 
    optFlags = [
        ["force", "f",
         "Re-use an existing directory (will not overwrite master.cfg file)"],
        ]
    ...
    def getSynopsis(self):
        return "Usage:    buildbot create-detector [options] <basedir>"

    longdesc = """
    This command creates a detector master working directory and buildbot.tac 
    file. The master will live in <dir> and create various files there.

    At runtime, the master will read a configuration file (named
    'detector.cfg' by default) in its basedir. This file should contain python
    code which eventually defines a dictionary named 'DetectormasterConfig'.
    The elements of this dictionary are used to configure the Detectormaster.
    See doc/config.xhtml for details about what can be controlled through
    this interface."""
...
detectorTAC = """
from twisted.application import service
from buildbot.master import BuggyChangelistDetectorMaster
...
application = service.Application('detectormaster')
try:
  from twisted.python.logfile import LogFile
  from twisted.python.log import ILogObserver, FileLogObserver
  logfile = LogFile.fromFullPath("twistd.log", rotateLength=rotateLength,
  ...
  # probably not yet twisted 8.2.0 and beyond, can't set log yet
  pass
BuggyChangelistDetectorMaster(basedir, configfile).setServiceParent(application)

"""

def createDetectorMaster(config):
    m = Maker(config)
    m.mkdir()
    m.chdir()
    contents = detectorTAC % config
    m.makeTAC(contents)
    m.sampleconfig(util.sibpath(__file__, "sample.cfg"))
    m.public_html({
          'index.html' : util.sibpath(__file__, "../status/web/index.html"),
          ...
          'buildbot.css' : util.sibpath(__file__, "../status/web/default.css"),
          'robots.txt' : util.sibpath(__file__, "../status/web/robots.txt"),
      })  #ratodo:  the web front UI should not be like the buildMaster waterfall
    m.makefile()

    if not m.quiet: print "detectormaster configured in %s" % m.basedir
...
class Options(usage.Options):
    ...
    subCommands = [
        ...
        ['create-detector-master', None, DetectorMasterOptions,
         "Create and populate a directory for a new detectormaster"], # ranew 20100203
...
def run():
    ...
    if command == "create-master":
        ...
        upgradeMaster(so)
    elif command == "create-detector-master":  #ranew
        createDetectorMaster(so)
